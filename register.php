...
if (isset($_POST['form_sent']))
{
	...
	$result = $db->query('SELECT 1 FROM user WHERE registration_ip=\''.$db->escape(get_remote_address()).'\' AND created_at>'.(time() - 3600)) or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
	...
	$result = $db->query('SELECT username FROM user WHERE email=\''.$db->escape($email1).'\'') or error('Unable to fetch user info', __FILE__, __LINE__, $db->error());
	if ($db->num_rows($result))
	{
